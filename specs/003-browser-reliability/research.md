...
- The first fix applied the single public `--max-cost-usd` cap to request, session, and daily ledger dimensions, failing closed instead of silently allowing cumulative overrun.
- Live Browserbase verification later found that applying the same value as the daily cap was too strict: after any same-day hosted spend, a fresh session could not reserve a later hosted call because `daily_total + max_cost_usd > max_cost_usd`.
- Red TDD tests `test_cloud_default_daily_cap_allows_fresh_session_after_prior_cost` and `test_cloud_configured_daily_cap_blocks_cross_session_call_before_provider` now pin the corrected split: `--max-cost-usd` remains the per-call/session cap, while `BFR_HOSTED_BROWSER_DAILY_COST_CAP_USD` controls cross-session daily spend.
- Whole-PR review then found Browserbase Stagehand does not emit `total_cost_usd`; releasing the no-reported-cost reservation made Browserbase daily/session caps a no-op in production. Red TDD tests now use the real Browserbase evidence shape and require successful no-reported-cost Browserbase runs to keep the conservative `--max-cost-usd` reservation so caps bind.
